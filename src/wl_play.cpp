...
#elif _XBOX
    bt_use, bt_nextweapon, bt_nobutton, bt_prevweapon, bt_nobutton, bt_nobutton, bt_run, bt_attack,
    bt_pause, bt_esc, bt_nobutton, bt_nobutton, bt_nobutton, bt_nobutton, bt_nobutton, bt_nobutton,
#else
...
// current user input
//
int controlx, controly, controlx2;         // range from -100 to 100 per tic
...
void PollJoystickMove (void)
{
    ...
    IN_GetJoyDelta (&joylx, &joyly, &joyrx, &joyry);

    int delta = buttonstate[bt_run] ? RUNMOVE * tics : BASEMOVE * tics;

    if (joyrx > 64 || buttonstate[bt_turnright])
        controlx += delta;
    else if (joyrx < -64  || buttonstate[bt_turnleft])
        controlx -= delta;
    if (joyly > 64 || buttonstate[bt_movebackward])
        controly += delta;
    else if (joyly < -64 || buttonstate[bt_moveforward])
        controly -= delta;
    if (joylx > 64)
		controlx2 += delta;
    else if(joylx < -64)
